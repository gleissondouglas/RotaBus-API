...
2. **Proatividade e Notificações (Push):** Utilizar Expo Notifications Push. Ex: "Você está perto do ponto do ônibus 100, ele passa em 5 min. Quer que eu te guie pra lá?".
3. **Administração Dinâmica (Painel):** Um CMS interno/Web-App que possibilite à gerência injetar aliases novos no banco de dados (`LocalIntelligence`), sem necessidade de dar commit/deploy do backend.
